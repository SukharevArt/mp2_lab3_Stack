...
TEST(TCalculator, can_calculate_expresion_with_plus)
{
	TCalculator v("3+2+23+45");
	EXPECT_EQ(v.Calc(),73);
}

TEST(TCalculator, can_calculate_expresion_with_minus)
{
	TCalculator v("32-2-565-32");
	EXPECT_EQ(v.Calc(),-567);
}

TEST(TCalculator, can_calculate_expresion_with_product)
{
...
TEST(TCalculator, can_calculate_expresion_with_division)
{
	TCalculator v("64/8");
	EXPECT_EQ(v.Calc(),8);
	v.SetExpr("3/3/2");
...
TEST(TCalculator, can_right_calculate_expresion)
{
